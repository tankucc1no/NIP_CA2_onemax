...
def generate_parameters():
    ...
    CROSSOVER_LENGTHS = random.sample(range(15), 15)
    MUTATION_SIZES = random.sample(range(15), 15)

    parameters = []
    for i in range(15):
        parameters.append([CROSSOVER_LENGTHS[i], MUTATION_SIZES[i]])
    return parameters


def parameters_tournament(parameters, mean, size=2):
    idx = random.sample(range(15), size)
    l = []
    for i in idx:
        l.append(mean[i])
    ...
    idx1 = idx[lst[0]]
    idx2 = idx[lst[1]]
    return parameters[idx1], parameters[idx2]
...
def weakest_replacement(parameter, record_iter, parameters, mean):
    if record_iter > 1000:
        return parameters, mean
    else:
        max_idx = np.array(mean).argsort()[0]
        if record_iter > mean[max_idx]:
            mean[max_idx] = record_iter
            parameters[max_idx] = parameter
        return parameters, mean
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        parameter1 = parameters_crossover_index(parameter1, parameter2)
        POP_SIZE = 100
        ...
        weakest_replacement(parameter1, sum_record_iter/100, parameters, mean)
